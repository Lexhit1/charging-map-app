...
RUN apt-get update \
 && apt-get install -y \
      ...
      libgmp-dev libcurl4-gnutls-dev libedit-dev \
      sqlite3 libsqlite3-dev postgresql-server-dev-all \
...
# Скачиваем, распаковываем и собираем PHP
...
# Копируем весь код, чтобы скрипты post-autoload могли найти artisan
COPY . .
...
# Копируем PHP, Composer и зависимости приложения
